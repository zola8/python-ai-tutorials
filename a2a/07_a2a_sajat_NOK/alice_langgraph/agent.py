...
def generate_alice_calendar() -> dict[str, list[str]]:
    """Generates Alice's calendar for the next 7 days."""
    calendar = {}
    today = date.today()
...
ALICE_CALENDAR = generate_alice_calendar()
...
class AvailabilityToolInput(BaseModel):
    ...
    date_range: str = Field(
        ...,
        description=(
            "The date or date range to check for availability, like"
...
def get_availability(date_range: str) -> str:
    ...
    try:
        ...
        for i in range(delta.days + 1):
            ...
            available_slots = ALICE_CALENDAR.get(date_str, [])
            if available_slots:
                availability = (
                    f"On {date_str}, Alice is available at: "
                    f"{', '.join(available_slots)}."
                )
                results.append(availability)
            else:
                results.append(f"Alice is not available on {date_str}.")
...
class AliceAgent:
    """AliceAgent - a specialized assistant for scheduling."""

    SUPPORTED_CONTENT_TYPES = ["text", "text/plain"]

    SYSTEM_INSTRUCTION = (
        "You are Alice's scheduling assistant. "
        "Your sole purpose is to use the 'get_availability' tool to answer questions about Alice's schedule for playing volleyball. "
        ...
        "If the user asks about anything other than scheduling volleyball, "
        ...
        for item in self.graph.stream(inputs, config, stream_mode="values"):
            ...
            if (
                ...
                yield {
                    "is_task_complete": False,
                    "require_user_input": False,
                    "content": "Checking Alice's availability...",
                }
...
if __name__ == "__main__":
    alice = AliceAgent()
    print("--------------------------------\nAlice's availability calendar: \n", json.dumps(ALICE_CALENDAR, indent=2))

    today = date.today()
    date_str = today.strftime("%Y-%m-%d")

    print("--------------------------------\nTool invoke:")
    print(alice.tools[0].invoke(date_str))
    tomorrow = today + timedelta(days=1)
    print(alice.tools[0].invoke(f"{today} to {tomorrow}"))
    print(alice.tools[0].invoke("1.1.2000"))
    print(alice.tools[0].invoke("hello"))
    print(alice.tools[0].invoke("2025-01-01"))

    # On 2025-08-27, Alice is available at: 18:00, 21:00.
    # I couldn't understand the date. Please ask to check availability for a date like 'YYYY-MM-DD'.
    # I couldn't understand the date. Please ask to check availability for a date like 'YYYY-MM-DD'.
    # Alice is not available on 2025-01-01.
    # On 2025-08-27, Alice is available at: 19:00, 20:00.
    # On 2025-08-28, Alice is available at: 18:00, 20:00.

    print("--------------------------------\nAgent invoke:")
    question = "Hello, are you free tomorrow?"
    print("- Hello, are you free tomorrow?")
    print("-", alice.invoke(question, context_id="123"))
