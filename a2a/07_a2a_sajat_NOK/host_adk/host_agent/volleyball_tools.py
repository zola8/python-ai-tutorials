...
def initialize_court_schedule():
    """Generates a schedule for the volleyball court for the next 7 days."""
...
initialize_court_schedule()


def list_court_availabilities(date: str) -> dict:
    """
    Lists the available and booked time slots for a volleyball court on a given date.
...
def book_volleyball_court(
        date: str, start_time: str, end_time: str, reservation_name: str
) -> dict:
    """
    Books a volleyball court for a given date and time range under a reservation name.
    ...
    return {
        "status": "success",
        "message": f"Success! The volleyball court has been booked for {reservation_name} from {start_time} to {end_time} on {date}.",
    }


if __name__ == "__main__":
    print("--------------------------------\nTool invoke:")
    print(list_court_availabilities("123"))
    print(list_court_availabilities("2000-01-01"))

    today = date.today()
    date_str = today.strftime("%Y-%m-%d")
    print(list_court_availabilities(date_str))

    print(book_volleyball_court("123", "12:00", "13:00", "Name1"))
    print(book_volleyball_court(date_str, "13:00", "12:00", "Name1"))

    print(book_volleyball_court(date_str, "12:00", "13:00", "Harry Potter"))
    print(book_volleyball_court(date_str, "12:00", "13:00", "Hermione Granger"))
    print(list_court_availabilities(date_str))

    # print("--------------------------------\nThe court's timetable:")
    # print(json.dumps(COURT_SCHEDULE, indent=2))
